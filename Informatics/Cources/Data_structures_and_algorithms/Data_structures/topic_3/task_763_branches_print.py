...
class Tree:
    ...
    def print_branches(self):
        if self.root:
            self._print_branches(node=self.root)
        return

    def _print_branches(self, node):
        if node:
            self._print_branches(node=node.left)
            if (node.left is not None) ^ (node.right is not None):
                print(node.key)
            self._print_branches(node=node.right)
        return
...
if __name__ == "__main__":
    ...
    if LOCAL_FILE:
        ...
        input_data_path = Path(__file__).parent.resolve() / 'task_763_input.txt'
