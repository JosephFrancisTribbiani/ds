...
from db import read_embeddings, mark_students
...
def make_card(metadata: dict, photo_loc: str, width: int, height: int, n_channels: int = 3) -> np.ndarray:
    ...
    cv.putText(img=card, text="Major:", org=(height + 15, anchor), fontFace=cv.FONT_HERSHEY_COMPLEX, 
    ...
    cv.putText(img=card, text="Attend.:", org=(height + 15, anchor), fontFace=cv.FONT_HERSHEY_COMPLEX, 
               fontScale=0.5, color=(0, 0, 0), thickness=1)
    ...
    cv.putText(img=card, text="Year:", org=(height + 15, anchor), fontFace=cv.FONT_HERSHEY_COMPLEX, 
               fontScale=0.5, color=(0, 0, 0), thickness=1)
    ...
    cv.putText(img=card, text="Status:", org=(height + 15, anchor), fontFace=cv.FONT_HERSHEY_COMPLEX, 
               fontScale=0.5, color=(0, 0, 0), thickness=1)
    ...
    while (cap.isOpened()):
        ...
        for face_meta, bbox in bboxes_to_plot:
            ...
            if face_meta == -1:
                ...
                # отметим
                marked = mark_students(ids=id)
                if marked: face_meta["status"] = "Marked"
