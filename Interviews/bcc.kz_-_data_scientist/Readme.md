...
обучена линейная модель - логистическая регрессию. Обучение производилось на 5-ти фолдах кросс валидации алгоритма ```GridSearchCV```. Наилучшая модель с параметрами: ```LogisticRegression(C=100, class_weight={0: 0.05, 1: 0.95}, solver='liblinear')```.
...
Для подбора оптимального порога по кривой ROC AUC использована метрика g-means. Данная метрика оптимальна в случае дисбаланса классов. Согласно данной метрике оптимальный порог - 0.2391.
...
precision | 0.23 | 0.25 | 0.18
recall | 0.40 | 0.43 | 0.33
ROC AUC | 0.89 | 0.87 | 0.90

ROC кривая и кривые FPR / FNR представлены на рисунке ниже:

<img src="images/roc_curve_boosting.png"></img>

Для подбора оптимального порога по кривой ROC AUC использована метрика g-means. Данная метрика оптимальна в случае дисбаланса классов. Согласно данной метрике оптимальный порог - 0.1496.
...
precision | 0.09 | 0.09 | 0.09
recall | 0.74 | 0.75 | 0.69
ROC AUC | 0.81 | 0.81 | 0.78

## Вывод

Идеальные результаты при решении данной задачи:
- precision, близкий к 1. Означает то, что все те клиенты, которых мы распознали как неплатежеспособных, на самом деле таковыми и являются. И мы не отказали в кредите тем клиентам, которые этот кредит выплатят. Т.е. банк получит от них свою выручку.
- recall, близкий к 1. Означает то, что мы в большинстве случаях не выдадим кредит тем клиентам, которые являются неплатежеспособными.

Обе модели показали схожие результаты. Решение о том, какую модель использовать в Prod должно основываться на размере выручки, которую банк получит / недополучит при использовании одной из этих двух моделей. Также, на основании этой стратегии должно приниматься решение о значении порога.