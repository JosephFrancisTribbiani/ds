...
# ToDo: assumption that width and height are relative to cell size is wrong. Actually this values are relative to whole image size.
#       so, Yolo loss and IoU calculation are wrong
#       in example is proposed to scale model output to cell ratio. Maybe it is neccesary to keep as is
...
class YoloLoss(nn.Module):
    ...
    def forward(self, prediction: torch.tensor, target: torch.tensor):
        ...
        # and if object exists in this cell
        wh_loss = self.wh_penalty(prediction=prediction, target=target, best_box_idx=bestbox)

        # Part 3:
        # this part penalize only anchor with highest IoU and if  the cell contains an object
        confidence_obj_loss = self.confidence_obj_penalty(prediction=prediction, target=target, best_box_idx=bestbox)

        # Part 4:
        # if a cell does not contain an object, then each anchor box in that cell which has confidence differ from 0 is penalized
        confidence_noobj_loss = self.confidence_noobj_penalty(prediction=prediction, target=target)

        # Part 5:

        return xy_loss + wh_loss + confidence_obj_loss + confidence_noobj_loss
    ...
    def wh_penalty(self, prediction: torch.tensor, target: torch.tensor, best_box_idx: torch.tensor) -> torch.tensor:
        """
        ...
        :param best_box_idx: torch tensor with index of anchor box which has maximum IoU with ground truth (size [batch size, split size, split size, 1]).
                             ...
                             taken into account for loss function calculation.
        """
        # extract w and h coordinates of best anchor boxes (which have highest IoU values)
        wh_of_best_anchor = torch.gather(input=prediction, dim=-1, index=best_box_idx*5 + torch.arange(2) + 2)
        object_exists = target[..., 4].unsqueeze(-1)

        # get sqrt with keeping sign
        # first we use abs to avoid square root from negative value, then return sign
        wh_of_best_anchor = torch.sqrt(torch.abs(wh_of_best_anchor))*torch.sign(wh_of_best_anchor)

        # calculate part of the YOLO loss function which is responcible for x and y coordinates
        return torch.sum(object_exists * torch.pow(wh_of_best_anchor - torch.sqrt(target[..., 2:4]), exponent=2)) * self.lambda_coord

    def confidence_obj_penalty(self, prediction: torch.tensor, target: torch.tensor, best_box_idx: torch.tensor) -> torch.tensor:
        """
        Calculates the third part of the YOLOv1 loss function, where penalize the confidence that the object is exists
        :param prediction: torch tensor with model predictions with size [batch size, split size, split size, 5*num_anchors + num_classes]
        :param target: tesor with target values with shape [batch_size, split_size, split size, 5 + num_classes]
        :param best_box_idx: torch tensor with index of anchor box which has maximum IoU with ground truth (size [batch size, split size, split size, 1]).
                             Value 1 in this tensor for example means that second anchor box has hieghtest IoU value with ground truth. Other anchor boxes are not
                             taken into account for loss function calculation.
        """
        confidence_of_best_anchor = torch.gather(input=prediction, dim=-1, index=best_box_idx*5 + 4)
        object_exists = target[..., 4].unsqueeze(-1)
        return torch.sum(object_exists * torch.pow(confidence_of_best_anchor - target[..., 4].unsqueeze(-1), exponent=2))

    def confidence_noobj_penalty(self, prediction: torch.tensor, target: torch.tensor) -> torch.tensor:
        noobj_exists = 1 - target[..., 4].unsqueeze(-1)
        return torch.sum(
            noobj_exists * torch.pow(
                target[..., 4].unsqueeze(-1) - prediction[..., torch.arange(0, 5*self.b, 5) + 4], 2)) * self.lambda_noobj
